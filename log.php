            ...
            <div id="content">
				...
				<form action="" method="post">
					...
					if($_SERVER["REQUEST_METHOD"] == "POST"){
						...
						$comnf = "nfdump -r /var/log/cgnat/flow/".$hostname."/".$mod_ano."/".$mod_mes."/".$mod_dia."/nfcapd.".$mod_ano.$mod_mes.$mod_dia.$hora."00 'src xport ".$Port." and src xip ".$Ip."'";
						$comzcatnf = "zcat /var/log/cgnat/flow/".$hostname."/".$mod_ano."/".$mod_mes."/".$mod_dia."/nfcapd.".$mod_ano.$mod_mes.$mod_dia.$hora."00 | nfdump -r - 'src xport ".$Port." and src xip ".$Ip."'";
						$comcat = "cat /var/log/cgnat/syslog/".$hostname."/".$mod_ano."/".$mod_mes."/".$mod_dia."/server-".$hora.".log | grep ".$Ip.":".$Port."| sed 's/<//g; s/>/->/g'";
						$comzcat = "zcat /var/log/cgnat/syslog/".$hostname."/".$mod_ano."/".$mod_mes."/".$mod_dia."/server-".$hora.".log | grep ".$Ip.":".$Port."| sed 's/<//g; s/>/->/g'";					
						if($tipo == "flow"){
							if($mod_data == $Data){
								echo "<pre class='text-primary'>";
								$output = shell_exec($comnf);
								echo $output;
								echo "</pre>";
							}else{
								echo "<pre class='text-primary'>";
								$output = shell_exec($comzcatnf);
								echo  $output;
								echo "</pre>";
							}
						}else{
							if($mod_data == $Data){
								echo "<pre class='text-primary'>";
								$output = shell_exec($comcat);
								echo $output;
								echo "</pre>";
							}else{
								echo "<pre class='text-primary'>";
								$output = shell_exec($comzcat);
								echo $output;
								echo "</pre>";
							}
						}					
